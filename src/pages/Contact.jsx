...
function Contact() {
    const [formData, setFormData] = useState({
      ...
      const handleSubmit = async (event) => {
        ...
        try {
          const response = await fetch('https://api.emailjs.com/api/v1.0/service/YOUR_SERVICE_ID/template/YOUR_TEMPLATE_ID', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              ...
              // Add any additional form fields here (optional)
            }),
          });
    
          if (response.ok) {
            setFormStatus('success'); // Indicate success
            setFormData({ name: '', email: '', message: '' }); // Clear form
          } else {
            setFormStatus('error'); // Indicate error
            throw new Error('Email sending failed');
          }
        } catch (error) {
          console.error(error);
        } finally {
          setFormStatus('idle'); // Reset state after operation
        }
      };
